...
class LeNet5(layers.ModuleWrapper):
    ...
    def __init__(self):
        ...
        # Conv-BN-Tanh-Pool
        if flags.var1:
            self.conv1 = layers.ConvVarianceUnif(1, int(20*flags.width), 5, padding=0, bias=False)
        else:
            self.conv1 = nn.Conv2d(1, int(20*flags.width), 5, padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(int(20*flags.width), affine=not flags.no_biases)
        self.relu1 = nonlinearity()
        self.pool1 = nn.MaxPool2d(2, padding=0)

        # Conv-BN-Tanh-Pool
        if flags.var2:
            self.conv2 = layers.ConvVarianceUnif(int(20*flags.width), int(50*width), 5, padding=0, bias=False)
        else:
            self.conv2 = nn.Conv2d(int(20*flags.width), int(50*flags.width), 5, padding=0, bias=False)
        self.bn2 = nn.BatchNorm2d(int(50*flags.width), affine=not flags.no_biases)
        ...
        # Dense-BN-Tanh
        if flags.var3:
            self.dense1 = layers.LinearVarianceUnif(int(800*flags.width), int(500*flags.width), bias=False)
        else:
            self.dense1 = nn.Linear(int(800*flags.width), int(500*flags.width), bias=False)
        self.bn3 = nn.BatchNorm1d(int(500*flags.width), affine=not flags.no_biases)
        self.relu3 = nonlinearity()

        # Dense
        if flags.var4:
            self.dense2 = layers.LinearVarianceUnif(int(500*flags.width), 10, bias=False)
        else:
